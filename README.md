...
LiveDialogs поддерживает **va_args**, вы можете форматировать текст, для этого подключите va_args из библиотеки YSI и тогда форматирование будет работать, иначе форматируйте самостоятельно:
...
            // С данной функцией форматирование будет 1 раз
...
- **`DIALOG_BACK`** — возвращает на один уровень выше, повторно обрисовывая диалог;
*  **`return DialogGoto:<deep>;`** откроет указанный диалог уровнем ниже. 
...
**DialogQuery** кэширует запросы, кэш удалится в том случае если окажется ниже по иерархии
```c
					...
					ListItem:<nickname>
					{
						Create:<"Удаление аккаунта">
						{
							...
							MessageBox:<str>
							{
								...
								// Так как мы перемещаем пользователя в диалог с кэшем, кэш не будет удалён, его нужно удалить самостоятельно что бы получить новую информацию.
								Dialog_CacheDestroy(playerid, deepid);
								return DialogGoto:<deepid>; // пример использования goto
							}
						}
